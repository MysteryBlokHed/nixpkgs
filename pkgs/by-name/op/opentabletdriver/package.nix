...
let
  ...
  dotnet-sdk = dotnetCorePackages.sdk_8_0;
    ...
    env.OTD_CONFIGURATIONS = "${src}/OpenTabletDriver.Configurations/Configurations";
    env.SELF_CONTAINED = stdenvNoCC.hostPlatform.isDarwin;
  ...
  darwin = buildDotnetModule (finalAttrs: {
    inherit
      pname
      version
      src
  ...
  postFetch = ''
    find . -type f -exec sed -i 's/osx-x64/osx-arm64/g' {} +
  '';
  ...
  ls -al $out/lib
      ...
      mv $out/bin/OpenTabletDriver.UX.MacOS $out/bin/otd-gui
  ...
  linux = buildDotnetModule (finalAttrs: {
    inherit
      pname
      version
      src
